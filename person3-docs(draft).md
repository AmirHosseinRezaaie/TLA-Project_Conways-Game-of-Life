...
### How to Run the Busy Beaver Simulations:
- **Standalone Simulator (`busy_beaver.py`):** Runs the simplified, self-contained Turing machine simulator.
  ```powershell
  python "Part 1 - Busy Beaver\busy_beaver.py"
  ```
Note: To modify state of busy beaver machine (to modfiy n in bb(n)) change it in the last line of `busy_beaver.py`. Also for running champion machines, comment the specific self-implemented BB and un-comment the provided champion BB.

- **Modular Test Suite (`test_busy_beaver_2.py`):** Runs the machines using the project's standard modular engine from `turing_machine.py`.
  ```powershell
  python "Part 1 - Busy Beaver\test_busy_beaver_2.py"
  ```
...
Yes, the output is correct. The machine successfully halts, leaving exactly four 1s on the tape and taking 6 steps to complete the computation, matching the trace demonstrated in **Task 3d**.
...
- Compare results with your designs
  ...
  - **BB(4):** My sweeping pattern achieved 10 steps (writing six `1`s), which is nowhere near the champion's staggering 107 steps (writing thirteen `1`s).
    - **Conclusion:** My designs prioritize predictable, guaranteed-to-halt **sweeping patterns**. A sweeping pattern is a systematic behavior where the machine's head traverses back and forth across a growing block of `1`s, expanding it at the boundaries on each sweep until a halting condition is met. While this makes behavior predictable and guarantees halting, it yields linear step counts and is sub-optimal compared to champion Busy Beavers. Champion machines utilize highly chaotic, Collatz-like transition rules to maximize tape usage and execution time before finally halting.

**Official Busy Beaver Champions (Halting Step Benchmarks)**:
These values represent the maximum number of steps (or shifts, $S(n)$) that the proven optimal $n$-state Busy Beaver machines take to halt:
- **2-state champion:** 6 steps (produces four `1`s)
- **3-state champion:** 21 steps (produces six `1`s)
- **4-state champion:** 107 steps (produces thirteen `1`s)
...
Was implemented. Note that `langton.py` is a library module that defines the core simulation class `LangtonsAnt` and has no standalone runner block; instead, the simulation is executed and visualized by running `langton_pygame.py`.
...
```
**Run the langton's ant:**
```powershell
python "Part 2 - GoL & Langton's Ant\langton_pygame.py"
```
For seeing results faster use --steps-per-frame flag.
